...
module.exports = {
	...
	data: new SlashCommandBuilder()
		.setName('포인트-송금')
		.setDescription('특정 대상에게 포인트를 송금 합니다')
	...
	async execute(interaction, client) {
        ...
        if(!interaction.options._hoistedOptions[0].user.bot)
        {
            ...
            //잔액 확인
            if(value <= data.Users[interaction.user.id].point)
            {
                //Data에서 포인트 이동
                data.Users[interaction.user.id].point -= value
                ...
                const embed2 = new MessageEmbed()
                    .setTitle('송금 성공 ✅')
                    .setDescription(`${targetName} 님에게 ${value} 포인트 를 송금했습니다`)
